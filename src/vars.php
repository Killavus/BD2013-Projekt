...
function calculate_ex(&$str, $from, $to, $level) //kod 'MEGA GÓWNO'
{
	...
	if($str[$from]=='*'||$str[$from]=='/'||$str[$from]=='%'||$str[$from]=='+'||$str[$from]=='-'||$str[$from]=='^'||
	   $str[$from]=='<'||$str[$from]=='>'||$str[$from]=='='||$str[$from]=='!'||$str[$from]=='&'||$str[$from]=='|') throw new Exception('op_at_beginning');
	if($str[$to]=='*'||$str[$to]=='/'||$str[$to]=='%'||$str[$to]=='+'||$str[$to]=='-'||$str[$to]=='^'||
	   $str[$to]=='<'||$str[$to]=='>'||$str[$to]=='='||$str[$to]=='!'||$str[$to]=='&'||$str[$to]=='|') throw new Exception('op_at_end');
